 ...
 int i_a,i_b,i_c,i_d,i_e, i_j;
   ...
   for (a=0; a< L1[x][0]; a++)
   {
	...
	//scanf("%d",&a);
	 ...
	 int counter,ind,f,g,i_f,i_g,l,i_l,flag1; 
	 for (x=0; x < n_g; x++)
	   for (a=0; a < L1[x][0]; a++)
		 for (b=0; b < L1[x][0]; b++)
		   for (c=0; c < L1[x][0]; c++)
		...
		// Fourth rounds of eliminations takes (x,a,b,c) & (y,a',b',c') and where let d in Dis(x,a,b,c) and d' in Dis(y,a,b,c) so //that more over (a,a'),(b,b'),(c,c') in L2(x,y). If there is some (z,a'',b'',c'') such that for e
